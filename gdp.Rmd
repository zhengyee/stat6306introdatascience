...
Required packages: RCurl. Install and/or load these packages before trying the code below.
```{r}
library(RCurl)
```
...
download.file(fileurl2,destfile='counry.csv',method='libcurl')
country<-read.csv('country.csv',header = T,stringsAsFactors = F)[,c(1,3)]
#Exclude the observations which have Income.Group blank.
country1<-subset(country,Income.Group!='')
```

##1. Match the data based on the country shortcode. How many of the IDs match? 
```{r}
a<-match(gdp1$CountryCode,country1$CountryCode,nomatch=NA_integer_)
length(which(a!='NA'))
```
...
```{r}
gdp3<-merge(gdp1,country1,by='CountryCode')
